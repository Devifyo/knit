...
// Reveal-on-scroll directive (dependency-free; reduced-motion handled in CSS).
const vReveal = {
    mounted(el, binding) {
...
// App-shell nav mock (hero visual)
const navMock = [
...
// Every capability, grouped — the full feature surface.
const groups = [
    ...
    { name: 'Automation', items: ['Visual workflow builder', 'Event triggers', 'Wait / delay steps', 'Conditions & branching', 'Tasks & reminders', 'Runs on a reliable queue'] },
...
const groupIcon = (i) => [
    'M16 11a4 4 0 10-8 0 4 4 0 008 0zM4 21a6 6 0 0112 0', // crm
    'M3 7h18l-2 13H5L3 7z', // sales
    'M6 3v12M6 21a3 3 0 100-6 3 3 0 000 6zM18 9a3 3 0 100-6 3 3 0 000 6zm0 0a9 9 0 01-9 9', // automation
    'M22 12h-6l-2 3h-4l-2-3H2M5 5l-3 7v6a2 2 0 002 2h16a2 2 0 002-2v-6l-3-7z', // comms
    'M3 9a2 2 0 012-2h14a2 2 0 012 2v2a2 2 0 000 4v2a2 2 0 01-2 2H5a2 2 0 01-2-2v-2a2 2 0 000-4V9z', // support
    'M3 8l8 5 8-5M5 5h14a2 2 0 012 2v10a2 2 0 01-2 2H5a2 2 0 01-2-2V7a2 2 0 012-2z', // marketing
    'M12 3l1.9 5.8L20 10l-6.1 1.2L12 17l-1.9-5.8L4 10l6.1-1.2z', // ai
    'M3 3v18h18M7 13l3-3 3 3 5-6', // analytics
    'M3 7a2 2 0 012-2h4l2 2h8a2 2 0 012 2v8a2 2 0 01-2 2H5a2 2 0 01-2-2V7z', // collab
    'M12 3l8 4v5c0 5-3.5 8-8 9-4.5-1-8-4-8-9V7l8-4zM9 12l2 2 4-4', // security
    'M2 7h20v12a2 2 0 01-2 2H4a2 2 0 01-2-2V7zM2 7l2-3h16l2 3', // billing
    'M12 2l9 5-9 5-9-5 9-5zM3 12l9 5 9-5', // modules
][i];
</script>
    ...
    <div class="min-h-[100dvh] bg-canvas text-ink">
        ...
        <section class="relative overflow-hidden px-5 pb-8 pt-32 lg:px-8 lg:pt-40">
            ...
            <div class="mx-auto max-w-[760px] text-center">
                <span class="rise inline-flex items-center gap-2 rounded-full border border-hairline bg-surface/70 px-3 py-1 text-xs font-medium text-muted backdrop-blur" style="animation-delay:.04s">
                    <span class="size-1.5 rounded-full" :style="{ background: 'var(--brand)' }" /> One workspace · 12 connected modules
                </span>
            ...
            <div id="product" class="rise mx-auto mt-12 max-w-[1040px]" style="animation-delay:.32s">
              <div class="float">
                <div class="overflow-hidden rounded-2xl border border-hairline bg-surface shadow-e3">
                    ...
                    <div class="grid grid-cols-[64px_1fr] sm:grid-cols-[200px_1fr]">
                        <!-- sidebar -->
                        ...
                        <div class="bg-canvas p-3 sm:p-4">
                            <div class="mb-3 flex items-center justify-between">
                                <div class="h-3 w-28 rounded-full bg-ink/15" />
                                <div class="h-6 w-20 rounded-md" :style="{ background: 'var(--brand)' }" />
                            </div>
                            <div class="grid grid-cols-3 gap-2.5">
                                <div v-for="(col, ci) in [['New', 3], ['Qualified', 2], ['Won', 2]]" :key="ci" class="rounded-xl bg-sunken/70 p-2">
                                    ...
                                    <div v-for="k in col[1]" :key="k" class="pop mb-1.5 rounded-lg border border-hairline bg-surface p-2 shadow-e1" :style="{ animationDelay: (0.6 + ci*0.1 + k*0.07) + 's' }">
                                        <div class="h-1.5 w-3/4 rounded-full bg-ink/15" />
                                        <div class="mt-2 flex items-center justify-between">
                                            <div class="h-1.5 w-9 rounded-full" :style="{ background: 'color-mix(in srgb, var(--brand) 55%, transparent)' }" />
                                            <div class="size-3.5 rounded-full bg-ink/10" />
                                        </div>
                                    </div>
                                </div>
                            </div>
        ...
        <section class="px-5 pb-16 pt-10 lg:px-8">
            <div class="mx-auto max-w-[1180px]">
                ...
                <div class="grid auto-rows-[200px] grid-cols-1 gap-4 md:grid-cols-6">
                    <!-- Pipeline (big) -->
                    <div v-reveal class="group relative overflow-hidden rounded-2xl border border-hairline bg-surface p-6 shadow-e1 transition-shadow hover:shadow-e2 md:col-span-4 md:row-span-2">
                        <h3 class="text-[15px] font-semibold tracking-[-0.01em]">Live deal pipeline</h3>
                        <p class="mt-1.5 max-w-sm text-sm text-muted">Drag deals across stages — changes sync to every teammate instantly over WebSockets.</p>
                        <div class="mt-5 grid grid-cols-3 gap-2.5">
                            <div v-for="(col, ci) in [['New', 3], ['Proposal', 2], ['Won', 2]]" :key="ci" class="rounded-xl bg-sunken/60 p-2">
                                <div class="mb-2 px-1 text-[9px] font-semibold uppercase tracking-wide text-faint">{{ col[0] }}</div>
                                <div v-for="k in col[1]" :key="k" class="mb-1.5 rounded-lg border border-hairline bg-surface p-2 shadow-e1 transition-transform group-hover:-translate-y-0.5" :style="{ transitionDelay: (ci*40 + k*30) + 'ms' }">
                                    <div class="h-1.5 w-4/5 rounded-full bg-ink/15" />
                                    <div class="mt-1.5 h-1.5 w-8 rounded-full" :style="{ background: 'color-mix(in srgb, var(--brand) 55%, transparent)' }" />
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <!-- Inbox -->
                    <div v-reveal="60" class="group overflow-hidden rounded-2xl border border-hairline bg-surface p-6 shadow-e1 transition-shadow hover:shadow-e2 md:col-span-2">
                        <h3 class="text-[15px] font-semibold tracking-[-0.01em]">Shared inbox</h3>
                        <p class="mt-1.5 text-sm text-muted">Threaded email, internal notes & @mentions.</p>
                        <div class="mt-4 space-y-1.5">
                            <div v-for="(w, i) in [70, 90, 55]" :key="i" class="flex items-center gap-2">
                                <div class="size-5 shrink-0 rounded-full bg-ink/10" />
                                <div class="h-1.5 rounded-full bg-ink/15" :style="{ width: w + '%' }" />
                            </div>
                        </div>
                    </div>

                    <!-- AI -->
                    <div v-reveal="120" class="relative overflow-hidden rounded-2xl border border-hairline bg-surface p-6 shadow-e1 transition-shadow hover:shadow-e2 md:col-span-2">
                        <div class="absolute -right-6 -top-6 size-24 rounded-full opacity-20 blur-2xl" :style="{ background: 'var(--brand)' }" />
                        <h3 class="text-[15px] font-semibold tracking-[-0.01em]">AI assist</h3>
                        <p class="mt-1.5 text-sm text-muted">Lead scoring, summaries & reply drafts.</p>
                        <div class="mt-4 inline-flex items-center gap-2 rounded-lg brand-wash px-2.5 py-1.5 text-[12px] font-medium text-[var(--brand)]">
                            <svg viewBox="0 0 24 24" fill="currentColor" class="size-3.5"><path d="M12 2l1.9 5.8L20 9.7l-6.1 1.9L12 17l-1.9-5.4L4 9.7l6.1-1.9z" /></svg>
                            Score: 87 · strong fit
                        </div>
                    </div>

                    <!-- Automation -->
                    <div v-reveal class="overflow-hidden rounded-2xl border border-hairline bg-surface p-6 shadow-e1 transition-shadow hover:shadow-e2 md:col-span-3">
                        ...
                        <div class="mt-4 flex items-center gap-1.5">
                            <span v-for="(s, i) in ['Trigger','Wait','If','Send']" :key="s" class="flex items-center gap-1.5">
                                <span class="rounded-lg border border-hairline bg-sunken px-2.5 py-1 text-[11px] font-medium text-ink-soft">{{ s }}</span>
                                <svg v-if="i < 3" viewBox="0 0 24 24" fill="none" class="size-3.5 text-faint" stroke="currentColor" stroke-width="2"><path d="M5 12h14M13 6l6 6-6 6" stroke-linecap="round" stroke-linejoin="round"/></svg>
                            </span>
                        </div>
                    </div>

                    <!-- Analytics -->
                    <div v-reveal="60" class="overflow-hidden rounded-2xl border border-hairline bg-surface p-6 shadow-e1 transition-shadow hover:shadow-e2 md:col-span-3">
                        <h3 class="text-[15px] font-semibold tracking-[-0.01em]">Analytics & reports</h3>
                        <p class="mt-1.5 text-sm text-muted">Live KPIs, exportable to CSV, Excel or PDF.</p>
                        <div class="mt-5 flex h-16 items-end gap-2">
                            <div v-for="(b, i) in [40, 65, 50, 80, 60, 95, 72]" :key="i" class="flex-1 rounded-t-md transition-all" :style="{ height: b + '%', background: 'color-mix(in srgb, var(--brand) ' + (40 + b/2) + '%, transparent)' }" />
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="mx-auto max-w-[1180px] space-y-20">
                ...
                <div v-reveal class="grid items-center gap-10 lg:grid-cols-2">
                    ...
                    <div class="rounded-2xl border border-hairline bg-surface p-5 shadow-e1">
                        <div v-for="(t, i) in [['Production is down','urgent','critical'],['How do I export?','normal','info'],['Billing question','low','neutral']]" :key="i" class="flex items-center justify-between border-b border-hairline-soft py-3 last:border-0">
                            <div class="flex items-center gap-3">
                                <span class="size-2 rounded-full" :class="{critical:'bg-critical',info:'bg-info',neutral:'bg-faint'}[t[2]]" />
                                <div class="h-2 w-32 rounded-full bg-ink/15" />
                            </div>
                ...
                <div v-reveal class="grid items-center gap-10 lg:grid-cols-2">
                    <div class="order-2 rounded-2xl border border-hairline bg-surface p-5 shadow-e1 lg:order-1">
                        <div class="grid grid-cols-3 gap-2">
                            <div v-for="(c, ci) in [['To do',2],['Doing',1],['Done',2]]" :key="ci" class="rounded-xl bg-sunken/60 p-2">
                                <div class="mb-2 px-1 text-[9px] font-semibold uppercase tracking-wide text-faint">{{ c[0] }}</div>
                                <div v-for="k in c[1]" :key="k" class="mb-1.5 rounded-lg border border-hairline bg-surface p-2 shadow-e1">
                                    <div class="h-1.5 w-4/5 rounded-full bg-ink/15" />
                                    <div class="mt-1.5 flex items-center gap-1"><div class="size-3 rounded-full bg-ink/10" /><div class="h-1.5 w-6 rounded-full bg-ink/10" /></div>
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        <!-- ============ SECURITY STRIP ============ -->
        <section class="px-5 pb-8 lg:px-8">
            <div v-reveal class="mx-auto flex max-w-[1180px] flex-col items-start justify-between gap-6 rounded-2xl border border-hairline bg-surface p-8 shadow-e1 sm:flex-row sm:items-center">
                <div class="flex items-center gap-4">
                    <span class="grid size-12 shrink-0 place-items-center rounded-xl brand-wash text-[var(--brand)]">
                        <svg viewBox="0 0 24 24" fill="none" class="size-6" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round"><path d="M12 3l8 4v5c0 5-3.5 8-8 9-4.5-1-8-4-8-9V7l8-4zM9 12l2 2 4-4" /></svg>
                    </span>
                    <div>
                        <h3 class="text-[15px] font-semibold tracking-[-0.01em]">Secure & compliant by default</h3>
                        <p class="mt-1 text-sm text-muted">Strict tenant isolation, RBAC, 2FA enforcement, IP allow-listing, audit trails and GDPR tooling.</p>
                    </div>
                </div>
                <div class="flex flex-wrap gap-2">
                    <span v-for="b in ['Multi-tenant','2FA','Audit log','GDPR','RBAC']" :key="b" class="rounded-full bg-sunken px-3 py-1 text-xs font-medium text-muted ring-1 ring-hairline">{{ b }}</span>
                </div>
            </div>
        </section>
...
@media (prefers-reduced-motion: reduce) {
    .rise, .pop, .float, .aurora { animation: none; }
    .rise, .pop { opacity: 1; transform: none; }
