            ...
            <div class="rounded-2xl border border-hairline bg-surface p-7 shadow-e1">
                ...
                <template v-else>
                    ...
                    <form class="mt-5 space-y-4" @submit.prevent="submit">
                        <Input v-for="f in form.fields" :key="f.key" v-model="data[f.key]" :label="f.label + (f.required ? ' *' : '')" :type="f.type === 'datetime' ? 'datetime-local' : (['email', 'number', 'tel', 'date'].includes(f.type) ? f.type : 'text')" :error="data.errors[f.key]" />
